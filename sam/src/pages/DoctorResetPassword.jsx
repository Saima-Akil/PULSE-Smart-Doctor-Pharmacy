...
const DoctorResetPassword = () => {
  ...
  const onSubmitEmail = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/auth/doctor-send-reset-otp', { email })
  ...
  const onSubmitOtp = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/auth/doctor-verify-reset-otp', { 
        email, 
        otp: otpValue 
      })
  ...
  const onSubmitPassword = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/auth/doctor-reset-password', { 
        email, 
        otp, 
        newPassword 
      })
    ...
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-sky-100 to-blue-400'>
      ...
      {!isEmailSent && (
        <form onSubmit={onSubmitEmail} className='bg-white p-8 rounded-lg shadow-lg w-full sm:w-96 text-sm'>
          <h1 className='text-indigo-900 text-2xl font-semibold text-center mb-4'>Doctor Reset Password</h1>
          <p className='text-center mb-6 text-indigo-400'>Enter your registered doctor email address</p>
          ...
          <p className='text-gray-400 text-center text-xs mt-4'>
            ...
            <span onClick={() => navigate('/login-as-doctor')} className='text-blue-500 cursor-pointer underline hover:text-blue-900'>
      ...
      {!isOtpSubmitted && isEmailSent && (
        <form onSubmit={onSubmitOtp} className='bg-white p-8 rounded-lg shadow-lg w-full sm:w-96 text-sm'>
          ...
          <p className='text-center mb-6 text-indigo-500'>Enter the 6-digit code sent to your doctor email</p>
      ...
      {isOtpSubmitted && isEmailSent && (
        <form onSubmit={onSubmitPassword} className='bg-white p-8 rounded-lg shadow-lg w-full sm:w-96 text-sm'>
          <h1 className='text-indigo-900 text-2xl font-semibold text-center mb-4'>Create New Doctor Password</h1>
          <p className='text-center mb-6 text-indigo-400'>Enter your new secure doctor password</p>

          <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
            <img src={assets.lock_icon} alt="" className='w-4 h-4'/>
            <input 
              type="password" 
              placeholder="New Doctor Password (min 6 characters)" 
              className='bg-transparent outline-none text-white flex-1' 
              value={newPassword} 
